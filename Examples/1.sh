...
for DATASET in "${DATASETS[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        ...
        # 检查并重命名11.csv为数据集名.csv，并移动到csv文件夹
        if [ -f "11.csv" ]; then
            NEW_CSV_PATH="$CSV_FOLDER/${DATASET}.csv"
            mv 11.csv "$NEW_CSV_PATH"
            echo "Renamed 11.csv to $NEW_CSV_PATH"
        else
            echo "Warning: 11.csv not found for dataset $DATASET"
        fi

        # 检查并重命名12.csv为数据集名.csv，并移动到csv文件夹
        if [ -f "12.csv" ]; then
            NEW_KP_CSV_PATH="$KP_FOLDER/${DATASET}.csv"
            mv "12.csv" "$NEW_KP_CSV_PATH"
            echo "Renamed $KP_FOLDER/12.csv to $NEW_KP_CSV_PATH"
        else
            echo "Warning: $KP_FOLDER/12.csv not found for dataset $DATASET"
        fi
