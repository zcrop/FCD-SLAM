...
int main(int argc, char *argv[])
{
    ...
    // Create SLAM system. It initializes all system threads and gets ready to process frames.
        //创建slam系统的开始时间
    std::cout << "创建slam系统的开始时间"<<std::endl;
    auto start_time = std::chrono::high_resolution_clock::now();
    ORB_SLAM3::System SLAM(argv[1],argv[2],ORB_SLAM3::System::IMU_MONOCULAR, true);
        //结束创建slam系统
auto end_time = std::chrono::high_resolution_clock::now();
    //输出中间的间隔时间
    std::cout << "创建slam系统的结束时间"<<std::endl;
auto elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
std::cout << "创建slam系统的执行时间：" << elapsed_time << " 毫秒" << std::endl;
    float imageScale = SLAM.GetImageScale();

    double t_resize = 0.f;//用于记录图像的缩放和跟踪的时间
    double t_track = 0.f;
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            double tframe = vTimestampsCam[seq][ni];//获得时间戳
            ...
            if(imageScale != 1.f)
            {
                ...
                cv::resize(im, im, cv::Size(width, height));//如果需要缩放就执行缩放操作
