...
{
  BowVector::iterator vit = this->lower_bound(id);//找到大于等于这个id的位置
  
  if(vit != this->end() && !(this->key_comp()(id, vit->first)))//如果相等就更新
  {
    vit->second += v;
  }
  else
  {
    this->insert(vit, BowVector::value_type(id, v));//否则就插入一个这样的结构
  }
}
