...
void Tracking::newParameterLoader(Settings *settings) {
    // 1. 读取相机1
    mpCamera = settings->camera1();//就返回一个变量
...
cout<<"num_t"<<num_t<<endl;
    ...
    if(mState==OK || mState==RECENTLY_LOST)
    {
        ...
        if(mCurrentFrame.isSet())
        {
            ...
            #ifdef ZRP_create_6
            if(mCurrentFrame.mpImuPreintegratedFrame->dW<0.004)
            {
                ORBmatcher matcher;
                matcher.setCheckOrientation(false);
            }
            else
            {
               // ORBmatcher::mbCheckOrientation=true;
               ORBmatcher matcher;
                matcher.setCheckOrientation(true);
            }
    #endif
            cout << "dW:" << mCurrentFrame.mpImuPreintegratedFrame->dW << endl;
        }
        else
        {
...
void Tracking::MonocularInitialization()
{
    ...
    else  // 第二帧来了
    {
        ...
        if(mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,Tcw,mvIniP3D,vbTriangulated))
        {
            ...
            {
                if(mvIniMatches[i]>=0 && !vbTriangulated[i])//存储三角化的点
                {
                    mvIniMatches[i]=-1;
                    nmatches--;
                }
