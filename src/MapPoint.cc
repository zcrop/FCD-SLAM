...
#include <cmath>
#define __U21_ZRP_DEL_LOWLEVEL
#include <opencv2/flann.hpp> // 包含FLANN库
// 其他包含文件...
    ...
    mnOriginMapId(pMap->GetId())
{
    ...
    unique_lock<mutex> lok(mpMap->mMutexPointCreation);
    mnId=nNextId++;
}
...
//删除不是单目的情况
#ifndef __U21_ZRP_DEL_LOWLEVEL //删除的标志
{
...
void MapPoint::SetBadFlag(bool erase)
{
    map<KeyFrame*, tuple<int,int>> obs;
    {
        ...
        obs = mObservations;
        // 把mObservations指向的内存释放，obs作为局部变量之后自动删除（函数执行完之后）
        mObservations.clear();
    }
    for(map<KeyFrame*, tuple<int,int>>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        ...
        #endif
        int leftIndex = get<0>(mit -> second);
        //==================================================
        if(leftIndex != -1){
...
#endif
#ifdef ZRP_create_3_1
/*
add 找到中值，就查找附近关键帧的距离
*/
void MapPoint::ComputeDistinctiveDescriptors()
{
    // Retrieve all observed descriptors
    ...
    vDescriptors.reserve(observations.size());
    // 相机中心向量容器
    vector<Eigen::Vector3f> cameraCenters;
    // Step 2 遍历观测到3d点的所有关键帧，获得orb描述子，并插入到vDescriptors中
    for(map<KeyFrame*,tuple<int,int>>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
    {
        // mit->first取观测到该地图点的关键帧
...
//==============================
 ...
 //===================================
            ...
            if(leftIndex != -1)
            {
                vDescriptors.push_back(pKF->mDescriptors.row(leftIndex));
                //添加一些相机中心点，世界坐标系
                cameraCenters.push_back(pKF->GetCameraCenter());
                //vDescriptors.push_back(pKF->mDescriptors.row(leftIndex));：这行代码从关键帧的描述子矩阵中提取位于 leftIndex 的行，即该地图点对应的特征描述子，并将其添加到描述子向量 vDescriptors 中。pKF->mDescriptors 是一个存储了该关键帧所有特征点描述子的矩阵，每一行对应一个特征点的描述子。

            }
            //删除右目
            #ifndef __U21_ZRP_DEL_LOWLEVEL
            if(rightIndex != -1)//不是单目，可以删除
            {
                vDescriptors.push_back(pKF->mDescriptors.row(rightIndex));
                cameraCenters.push_back(pKF->GetRightCameraCenter());
            }
            #endif
        }
    }
    ...
    if(vDescriptors.empty())
        return;
   Eigen::Vector3f worldPos = this->GetWorldPos();//世界坐标系
    // Compute distances between them
    // Step 3 获得这些描述子两两之间的距离
    // N表示为一共多少个描述子
    const size_t N = vDescriptors.size();
    ...
    for(size_t i=0;i<N;i++)
    {
        ...
        for(size_t j=i+1;j<N;j++)
        {
            int distij = ORBmatcher::DescriptorDistance(vDescriptors[i],vDescriptors[j]);//计算描述子的距离，这个不知道可不可以改成权重来计算
            Distances[i][j]=distij;
            Distances[j][i]=distij;
        }
    }
    ...
    int BestMedian = INT_MAX;
    int BestIdx = 0;
    vector<int> bestIndexes;//里面是包含中值的
    for(size_t i=0;i<N;i++)
    {
        // 第i个描述子到其它所有所有描述子之间的距离
        vector<int> vDists(Distances[i],Distances[i]+N);
...
//----------计算其中附近三个描述子方向相似度的-------------------
    // Calculate direction similarity for the median and its neighbors
    int indicesToCheck[3] = {BestIdx - 1, BestIdx, BestIdx + 1};
    float BestSimilarity = FLT_MAX;
    int MostRepresentativeIdx = BestIdx;
    float Max_similarity = 0;//最大相似度
    float similarity=0;//用来比较的数
    for (int i = 0; i < 3; i++) {
        int idx = indicesToCheck[i];
        if (idx < 0 || idx >= N) continue;  // Skip if index is out of range

        Eigen::Vector3f direction = cameraCenters[idx] - this->GetWorldPos();
        direction.normalize();//归一化
        similarity= acos(std::max(-1.0f, std::min(1.0f, direction.dot(mNormalVector.normalized()))));
        if(similarity>Max_similarity)//求最大的相似度
            {
                Max_similarity=similarity;
                MostRepresentativeIdx =idx;
            }
...
//-----------------------------------------------
    {
        ...
        // 简化来讲，中值代表了这个描述子到其它描述子的平均距离
        // 最好的描述子就是和其它描述子的平均距离最小
        mDescriptor = vDescriptors[MostRepresentativeIdx].clone();
    }
}
#endif
#ifdef ZRP_create_4
...
void MapPoint::ComputeDistinctiveDescriptors()
{
    ...
    // Step 2 遍历观测到3D点的所有关键帧，获得 ORB 描述子
    for (const auto& observation : observations)
    {
        ...
        if (!pKF->isBad())
        {
            int leftIndex = std::get<0>(observation.second);
            if (leftIndex != -1)
            {
                vDescriptors.push_back(pKF->mDescriptors.row(leftIndex));
            }
            int rightIndex = std::get<1>(observation.second);
            if (rightIndex != -1)
            {
                vDescriptors.push_back(pKF->mDescriptors.row(rightIndex));
            }
    ...
    // Step 3 将 vDescriptors 转换为 cv::Mat 类型
    cv::Mat matDescriptors;
    cv::vconcat(vDescriptors, matDescriptors);

    // Step 4 使用 LSH 构建 FLANN 索引
    cvflann::LshIndexParams lshParams(12, 20, 2); // 确保参数设置正确
    cvflann::Index<cvflann::HammingDistance> flannIndex(matDescriptors, lshParams);
    flannIndex.buildIndex();

    // Step 5 选择最有代表性的描述子，它与其他描述子应具有最小的距离中值
    int BestMedian = INT_MAX;
    int BestIdx = 0;

    for (size_t i = 0; i < vDescriptors.size(); i++)
    {
        std::vector<int> indices(10); // k = 10
        std::vector<float> dists(10);
        
        // 查询近似最近邻
        flannIndex.knnSearch(vDescriptors[i], indices, dists, 10, cvflann::SearchParams());

        // 计算与其他描述子距离的中值
        std::vector<float> vDists(dists.begin(), dists.end());
        std::sort(vDists.begin(), vDists.end());
        int median = vDists[vDists.size() / 2];
...

    {
        ...
        mDescriptor = vDescriptors[BestIdx].clone(); // 选定最佳描述子
    }
}
#endif

#ifdef ZRP_create_5//描述子修改方法
void MapPoint::ComputeDistinctiveDescriptors()
{
    // Retrieve all observed descriptors
    vector<cv::Mat> vDescriptors;
    ...
    // 相机中心向量容器
    vector<Eigen::Vector3f> cameraCenters;
    cameraCenters.reserve(observations.size());
    ...
    for(map<KeyFrame*,tuple<int,int>>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
    {
        ...
        if(!pKF->isBad())
        {
            ...
            // 原代码里 leftIndex 重复定义很多次，这里我们直接取一次
            int leftIndex  = get<0>(indexes);
            int rightIndex = get<1>(indexes);

            if(leftIndex != -1)
            {
                // ⚠ 改动1：在这里 clone，确保是独立的1x32 CV_8U行
                vDescriptors.push_back(pKF->mDescriptors.row(leftIndex).clone());
                cameraCenters.push_back(pKF->GetCameraCenter());
            }
            ...
            {
                // ⚠ 同理 clone
                vDescriptors.push_back(pKF->mDescriptors.row(rightIndex).clone());
                cameraCenters.push_back(pKF->GetRightCameraCenter());
            }
            #endif
        }
    ...
    // 世界坐标
    Eigen::Vector3f worldPos = this->GetWorldPos();

    // Step 3 计算两两描述子距离矩阵
    const size_t N = vDescriptors.size();

    // 你原来用了 float Distances[N][N]; 这是GCC扩展的变长数组
    // 我们保持你的风格，尽量不全重写结构
    float Distances[N][N];
    for(size_t i=0;i<N;i++)
    {
    ...
    for(size_t i=0;i<N;i++)
    {
        vector<int> vDists(Distances[i], Distances[i] + N);
        sort(vDists.begin(), vDists.end());
        int median = vDists[0.5*(N-1)]; // 你原来的写法我保持不动
    ...
    // Step 5 在 BestIdx 附近(BestIdx-1, BestIdx, BestIdx+1)里挑视角方向更合理的那个
    // （你的逻辑保留，只修"相似度"的计算方式）
// Step 5 在 BestIdx 附近(BestIdx-1, BestIdx, BestIdx+1)里挑视角方向更合理的那个

int indicesToCheck[3] = {BestIdx - 1, BestIdx, BestIdx + 1};

float Max_similarity = -2.0f;
int MostRepresentativeIdx = BestIdx;
float similarity = 0.0f;

Eigen::Vector3f normalDir = mNormalVector.normalized(); // 视线平均方向

for (int k = 0; k < 3; k++) {
    int idx = indicesToCheck[k];
    if (idx < 0 || idx >= (int)N) continue;

    Eigen::Vector3f direction = cameraCenters[idx] - worldPos;
    direction.normalize();

    similarity = direction.dot(normalDir); // cosθ，越接近1越同向
    ...
    // Step 6 把最终选出来的描述子写回 MapPoint
    {
        unique_lock<mutex> lock(mMutexFeatures);
        // clone() 是标准做法，MapPoint持有自己的主描述子
        mDescriptor = vDescriptors[MostRepresentativeIdx].clone();
    }
}
