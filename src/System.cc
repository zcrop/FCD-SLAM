...
/*没有选择了，直接是单目+IMU*/
#if defined(__U21_ZRP_DEL_LOWLEVEL)
{
       cout << "Input sensor was set to :Monocular-Inertial"<<endl;
...
//--------------------------------------------------------------
    ...
    if(mStrLoadAtlasFromFile.empty())//没有加载到地图
    {
        //Load ORB Vocabulary
        ...
        mpAtlas = new Atlas(0);//其中就是 创建地图createNewmap
    }
    else//加载到地图文件
    {
        //Load ORB Vocabulary
    ...
    if (mSensor==IMU_STEREO || mSensor==IMU_MONOCULAR || mSensor==IMU_RGBD)
        mpAtlas->SetInertialSensor();//标志IMU
//-----------------前面的与单双目无关-----------------------------------------------------------------------
...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{

    {
        unique_lock<mutex> lock(mMutexReset);
        if(mbShutDown)
            return Sophus::SE3f();//检查是否关闭，关闭返回默认位姿
    }
    // 确保是单目或单目VIO模式
    ...
    if (mSensor == System::IMU_MONOCULAR)
        for(size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
            mpTracker->GrabImuData(vImuMeas[i_imu]);//把数据存储在队列的函数
...
#ifdef ZRP_create_write
writeIntToCSV(num_t,"gg.csv");
#endif
...
#ifdef ZRP_create_write
void System::writeIntToCSV(int value, const std::string& filename) {
