            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        ...
                        cout<<"内点数量----------------------"<<mpTracker->GetMatchesInliers()<<endl;
                        cout<<"特征点数量----------------------"<<nkeypoints<<endl;
                        cout<<"num_t"<<num_t<<endl;
                        writeIntToCSV(mpTracker->GetMatchesInliers(),"11.csv");
                        writeIntToCSV(nkeypoints,"12.csv");
                        writeIntToCSV(num_t,"13.csv");
         ...
         if(pMP && !pMP->isBad())
            {
                if(!pMP->IsInKeyFrame(mpCurrentKeyFrame))
                {
                    ...
                    pMP->UpdateNormalAndDepth();
                    // 更新地图点的最佳描述子
                    pMP->ComputeDistinctiveDescriptors();//改变了一个点
...
void LocalMapping::CreateNewMapPoints()
{
    // Retrieve neighbor keyframes in covisibility graph
     int nn;//由于 nn 在条件编译块内声明，其作用域仅限于该块内部，因此在块外部引用 nn 会导致编译错误。
    #ifndef  __U21_ZRP_DEL_LOWLEVEL
   { // nn表示搜索最佳共视关键帧的数目
...
{        if(!mbMonocular)
        ...
        else
        {
            // 单目相机情况
            // 邻接关键帧的场景深度中值
            const float medianDepthKF2 = pKF2->ComputeSceneMedianDepth(2);//计算场景的深度中值
            // baseline与景深的比例
...
#else
{
                // 单目相机情况
            // 邻接关键帧的场景深度中值
            const float medianDepthKF2 = pKF2->ComputeSceneMedianDepth(2);//计算场景的深度中值,比较容易看懂
            // baseline与景深的比例
            const float ratioBaselineDepth = baseline/medianDepthKF2;
            // 如果特别远(比例特别小)，基线太短恢复3D点不准，那么跳过当前邻接的关键帧，不生成3D点
            if(ratioBaselineDepth<0.01)
        ...
        vector<pair<size_t,size_t> > vMatchedIndices;//一个存储匹配点索引，一个存储邻接关键帧
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            const float kp1_ur=mpCurrentKeyFrame->mvuRight[idx1];//有点问题
            bool bStereo1 = (!mpCurrentKeyFrame->mpCamera2 && kp1_ur>=0);//检查特征点是否有错误吗
            // 查看点idx1是否为右目的点
...
// 将单个整数写入 CSV 文件
void LocalMapping::writeIntToCSV(int value, const std::string& filename) {
