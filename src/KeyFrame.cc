...
void KeyFrame::ComputeBoW()
{
    ...
    {//将特征点的描述子转化为DBOW要求的格式
        vector<cv::Mat> vCurrentDesc = Converter::toDescriptorVector(mDescriptors);
        // Feature vector associate features with nodes in the 4th level (from leaves up)
        // We assume the vocabulary tree has 6 levels, change the 4 otherwise
        mpORBvocabulary->transform(vCurrentDesc, mBowVec, mFeatVec, 4);//当前的描述子，输出的词袋向量（记录单词的id以及对应的权重TF-IDF值，输出，记录node，id以及对应的图像 对应的索引，叶节点向前的层数），
    //将特征点的描述子转化为词袋向量以及特征向量
    }
}
...
void KeyFrame::UpdateConnections(bool upParent)
{
    ...
    for (vector<MapPoint *>::iterator vit = vpMP.begin(), vend = vpMP.end(); vit != vend; vit++)
    {
        ...
        {
            if (mit->first->mnId == mnId || mit->first->isBad() || mit->first->GetMap() != mpMap)//与自己不是共视
                continue;
            // 这里的操作非常精彩！
